 ...
 */
class Magetesting_Backup_Filesystem extends Magetesting_Backup_Abstract
{
    /**
    ...
    public function rollback()
    {
        ...
        $rollbackWorker = $this->_useFtp ? new Magetesting_Backup_Filesystem_Rollback_Ftp($this)
            : new Magetesting_Backup_Filesystem_Rollback_Fs($this);
        $rollbackWorker->run();
    ...
    public function create()
    {
        ...
        $fsHelper = new Magetesting_Backup_Filesystem_Helper();

        $filesInfo = $fsHelper->getInfo(
            $this->getRootDir(),
            Magetesting_Backup_Filesystem_Helper::INFO_READABLE | Magetesting_Backup_Filesystem_Helper::INFO_SIZE,
            $this->getIgnorePaths()
        );

        if (!$filesInfo['readable']) {
            throw new Magetesting_Backup_Exception_NotEnoughPermissions('Not enough permissions to read files for backup');
        }

        $freeSpace = disk_free_space($this->getBackupsDir());

        if (2 * $filesInfo['size'] > $freeSpace) {
            throw new Magetesting_Backup_Exception_NotEnoughFreeSpace('Not enough free space to create backup');
        }

        $tarTmpPath = $this->_getTarTmpPath();

        $tarPacker = new Magetesting_Backup_Archive_Tar();
        ...
        $gzPacker = new Magetesting_Archive_Gz();
     ...
     * @param string $path
     * @return Magetesting_Backup_Filesystem
     */
     ...
     *
     * @see Magetesting_Backup_Abstract::setBackupsDir()
    ...
    protected function _checkBackupsDir()
    {
        ...
        if (!is_dir($backupsDir)) {
            ...
            if (!is_writeable($backupsDirParentDirectory)) {
                throw new Magetesting_Backup_Exception_NotEnoughPermissions('Cant create backups directory');
            }
        ...
        if (!is_writable($backupsDir)) {
            throw new Magetesting_Backup_Exception_NotEnoughPermissions('Backups directory is not writeable');
        }
    }
